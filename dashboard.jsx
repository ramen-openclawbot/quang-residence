...
/* ═══════════════════════════════════════════
   ICON COMPONENTS (Material-style inline SVGs)
   ═══════════════════════════════════════════ */
...
const ThermoIcon = (p) => <Icon {...p}><path d="M14 14.76V3.5a2.5 2.5 0 00-5 0v11.26a4.5 4.5 0 105 0z" /></Icon>;
...
/* ═══════════════════════════════════════════
   ...
   ═══════════════════════════════════════════ */
const monthlyData = [
  { m: "Jan", spend: 3200, budget: 5000 }, { m: "Feb", spend: 3800, budget: 5000 },
  { m: "Mar", spend: 2600, budget: 5000 }, { m: "Apr", spend: 4200, budget: 5000 },
  { m: "May", spend: 3400, budget: 5000 }, { m: "Jun", spend: 2200, budget: 5000 },
];

const spendingItems = [
  { name: "Groceries", desc: "Weekly Restock", amt: 1240, color: "#f97316", bgColor: "#fff7ed", icon: ShoppingIcon },
  { name: "Bill Payments", desc: "Electricity & Water", amt: 450.20, color: "#3b82f6", bgColor: "#eff6ff", icon: ZapIcon },
  { name: "Online Subscriptions", desc: "Netflix, Spotify, iCloud", amt: 89.99, color: "#8b5cf6", bgColor: "#f5f3ff", icon: WifiIcon },
  { name: "Travel", desc: "Weekend Retreat", amt: 2800, color: "#22c55e", bgColor: "#f0fdf4", icon: PlaneIcon },
];

const driverTasks = [
  { id: 1, time: "14:00", dur: "2h", title: "Airport Transfer", loc: "Narita International Terminal 3", status: "scheduled", icon: PlaneIcon },
  { id: 2, time: "19:30", dur: "1h", title: "Evening Gala Drop-off", loc: "Imperial Hotel Main Entrance", status: "waiting", icon: CarIcon },
];

const secTasks = [
  { id: 3, time: "10:00", dur: "1h", title: "Review travel itinerary", loc: "Kyoto Autumn Season 2023", status: "in-progress", icon: MapIcon },
  { id: 4, time: "16:30", dur: "30m", title: "Sign Q4 budget report", loc: "Office Lounge / Remote", status: "pending", icon: EditIcon },
];

const weekDays = [
  { day: "Mon", num: 10 }, { day: "Tue", num: 11 }, { day: "Wed", num: 12 },
  { day: "Thu", num: 13, active: true }, { day: "Fri", num: 14 }, { day: "Sat", num: 15 },
];

const fmt = (n) => n.toLocaleString("en-US", { style: "currency", currency: "USD", minimumFractionDigits: n % 1 === 0 ? 0 : 2 });

/* ═══════════════════════════════════════════
   SHARED STYLES
   ═══════════════════════════════════════════ */
const card = { backgroundColor: T.card, border: `1px solid ${T.borderLight}`, borderRadius: 12, padding: 20, boxShadow: T.shadow };
const sectionLabel = { fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.12em", color: T.textLabel, marginBottom: 16 };
  ...
  <div style={{ padding: "0 24px", paddingBottom: 40 }}>
    ...
    <section style={{ marginBottom: 32 }}>
      <div style={{ ...flexBetween, marginBottom: 16 }}>
        ...
        <span onClick={() => onNav("wealth")} style={{ fontSize: 13, color: T.primary, fontWeight: 600, cursor: "pointer" }}>View Report</span>
      </div>
      <div style={card}>
        <div style={{ ...flexCenter, gap: 8, marginBottom: 16 }}>
          <span style={{ fontSize: 32, fontWeight: 300, color: T.text }}>$4,250</span>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <div><div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", color: T.textLabel, marginBottom: 4 }}>Groceries</div><div style={{ fontSize: 14, fontWeight: 600, color: T.text }}>$800</div></div>
          <div><div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", color: T.textLabel, marginBottom: 4 }}>Utilities</div><div style={{ fontSize: 14, fontWeight: 600, color: T.text }}>$950</div></div>
        </div>
      </div>
    </section>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div onClick={() => onNav("ambiance")} style={{ ...card, cursor: "pointer", padding: 16 }}>
          <div style={{ ...flexBetween, marginBottom: 12 }}>
            ...
            <span style={{ fontSize: 10, fontWeight: 700, color: T.primary, padding: "2px 8px", backgroundColor: T.primaryBg, borderRadius: 99 }}>60%</span>
          </div>
    ...
    <section>
      ...
      <div style={{ ...card, padding: 0, overflow: "hidden" }}>
        {[
          { type: "Driver", task: "Airport Transfer at 14:00", time: "2h left", icon: CarIcon },
          { type: "Secretary", task: "Sign Q4 budget report", time: "16:30", icon: FileIcon },
        ].map((item, i) => (
          <div key={i} onClick={() => onNav("agenda")} style={{ ...flexCenter, gap: 16, padding: 16, cursor: "pointer", borderBottom: i === 0 ? `1px solid ${T.border}` : "none" }}>
            <div style={{ width: 40, height: 40, borderRadius: 8, backgroundColor: "#f8fafc", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <item.icon size={20} color={T.textMuted} />
            </div>
            <div style={{ flex: 1 }}>
...
const WealthScreen = () => {
  ...
  const barHeights = [66, 75, 50, 86, 66, 50];
  const fillPcts = [50, 66, 33, 80, 50, 25];
  ...
  return (
    <div style={{ padding: "0 24px", paddingBottom: 40 }}>
      ...
      <div style={{ display: "flex", height: 44, backgroundColor: T.primaryBg, borderRadius: 12, padding: 4, border: `1px solid ${T.primaryBg2}`, marginBottom: 32 }}>
        {["month", "year"].map(p => (
          <button key={p} onClick={() => setPeriod(p)} style={{ flex: 1, borderRadius: 8, border: "none", cursor: "pointer", fontSize: 14, fontWeight: 500, backgroundColor: period === p ? T.card : "transparent", color: T.textSec, boxShadow: period === p ? T.shadow : "none", textTransform: "capitalize" }}>
            {p}
          </button>
        ))}
      </div>
      ...
      <div style={{ textAlign: "center", marginBottom: 32 }}>
        <div style={{ fontSize: 12, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.12em", color: T.textMuted, marginBottom: 4 }}>Total Net Worth</div>
      ...
      <div style={{ ...card, marginBottom: 32, padding: 24 }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", height: 128, gap: 8 }}>
          {monthlyData.map((d, i) => (
            <div key={i} style={{ flex: 1, backgroundColor: T.primaryBg2, borderRadius: "6px 6px 0 0", height: `${barHeights[i]}%`, position: "relative" }}>
              <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: `${fillPcts[i]}%`, backgroundColor: i === 3 ? T.primary : T.primary + "66", borderRadius: "6px 6px 0 0" }} />
            </div>
          ))}
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12, padding: "0 2px" }}>
          {monthlyData.map((d, i) => <span key={i} style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: T.textMuted + "66" }}>{d.m}</span>)}
        </div>
      </div>
            ...
            return (
              <div key={i} style={{ ...flexBetween, ...card, padding: 16 }}>
                <div style={{ ...flexCenter, gap: 16 }}>
                  <div style={{ width: 40, height: 40, borderRadius: "50%", backgroundColor: item.bgColor, display: "flex", alignItems: "center", justifyContent: "center" }}>
                ...
                <div style={{ fontSize: 15, fontWeight: 700, color: T.text }}>-{fmt(item.amt)}</div>
              </div>
            );
          })}
        </div>
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          {[
            { name: "Original Painting", date: "Acquired: Jan 2024", price: "$12,500", color: "#f59e0b" },
            { name: "Vintage Furniture", date: "Acquired: Mar 2024", price: "$4,200", color: "#8b5cf6" },
          ].map((inv, i) => (
            <div key={i} style={{ ...card, padding: 16 }}>
              ...
              <div style={{ fontSize: 10, color: T.textMuted, marginTop: 2, marginBottom: 8 }}>{inv.date}</div>
  ...
  return (
    <div style={{ padding: "0 24px", paddingBottom: 40 }}>
      ...
      <section style={{ marginBottom: 24 }}>
        ...
        <div style={{ ...card, ...flexBetween, padding: 24, borderRadius: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 40, fontWeight: 300, color: T.text }}>24<span style={{ color: T.primary }}>°C</span></div>
            <div style={{ ...flexCenter, gap: 4, fontSize: 12, color: T.textMuted, marginTop: 8 }}>
              <DropIcon size={12} color={T.textMuted} /> 45% Humidity
            </div>
          </div>
          <div style={{ position: "relative", width: 112, height: 112 }}>
            ...
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
              <span style={{ fontSize: 14, fontWeight: 700, color: T.text }}>22.5</span>
      ...
      <section>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          {[
            { name: "Air Purifier", desc: "Auto Mode", on: purifier, toggle: () => setPurifier(!purifier), color: "#3b82f6", bg: "#eff6ff", Ic: WindIcon },
            { name: "Smart Blinds", desc: "Closed", on: blinds, toggle: () => setBlinds(!blinds), color: "#f97316", bg: "#fff7ed", Ic: LightIcon },
          ].map((d, i) => (
...
const AgendaScreen = () => {
  ...
  return (
    <div style={{ padding: "0 24px", paddingBottom: 40 }}>
      ...
      <div style={{ ...flexBetween, marginBottom: 24 }}>
        <div>
          <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.12em", color: T.textMuted, fontWeight: 600 }}>March 2026</div>
          <div style={{ fontSize: 18, fontWeight: 500, color: T.text, marginTop: 4 }}>Thursday, 13th</div>
        </div>
            ...
            return (
              <div key={task.id} style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                <div style={{ width: 48, flexShrink: 0, paddingTop: 4, textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 10, color: T.textMuted }}>{task.dur}</div>
                </div>
                <div style={{ flex: 1, ...card, padding: 16 }}>
                  <div style={{ ...flexBetween, marginBottom: 4 }}>
                    ...
                    <task.icon size={18} color={T.primary} />
                  </div>
                  <div style={{ fontSize: 14, color: T.textMuted, marginBottom: 12 }}>{task.loc}</div>
                  <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", padding: "3px 8px", borderRadius: 4, backgroundColor: st.bg, color: st.color }}>{task.status.replace("-", " ")}</span>
                </div>
              </div>
            );
          })}
...
export default function Dashboard() {
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: T.bg, fontFamily: "'Manrope', 'Inter', -apple-system, sans-serif", maxWidth: 430, margin: "0 auto", position: "relative", boxShadow: "0 0 60px rgba(0,0,0,0.08)" }}>
      ...
      {/* Header */}
      {tab === "home" ? (
        <header style={{ ...flexBetween, padding: "32px 24px 24px" }}>
          <div style={{ ...flexCenter, gap: 12 }}>
