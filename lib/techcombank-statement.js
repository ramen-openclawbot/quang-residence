...
export function reconcileStatementWithTransactions(statementEntries = [], appTransactions = []) {
  ...
  for (const entry of effectiveEntries) {
    const codeMatchIdx = unmatchedApp.findIndex((tx) => {
      ...
      return !!entry.transaction_no && !!tx.transaction_code && String(entry.transaction_no).trim() === String(tx.transaction_code).trim() && sameDirection && txAmount === Number(entry.amount || 0);
    });
